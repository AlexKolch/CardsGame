...
class BoardGameController: UIViewController {
    ...
    private var flippedCards = [UIView]() //Значение в данном свойстве будут использоваться для сравнения идентичности карточек
    ...
    private func getCardsBy(modelData: [Card]) -> [UIView] {
        ...
        for cardView in cardViews {
            (cardView as! FlippableView).flipCompletionHandler = { [weak self] flippedCard in
                guard let self else {return}
                ...
                } else if let index = self.flippedCards.firstIndex(of: flippedCard) {
